<template>
  <div class="table-container">
    ...
    <el-table ref="tableSort" v-loading="listLoading" :data="list" :element-loading-text="elementLoadingText"
      :height="height" @selection-change="setSelectRows" @sort-change="tableSortChange">
    ...
    <table-edit ref="edit" :isEdit="isEdit" @fetch-data="fetchData" />
  </div>
</template>

<script>
import { doDelete } from '@/api/table'
import { tableList, tableDelete } from '@/api/index'
...
export default {
  ...
  methods: {
    ...
    handleDelete (row) {
      ...
      if (row.id) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          let res = await tableDelete({ ids: row.id })
          if (res.status == 200) {
        ...
        if (this.selectRows.length > 0) {
          ...
          this.$baseConfirm('你确定要删除选中项吗', null, async () => {
            let res = await doDelete({ ids: ids })
            if (res.status == 200) {
              this.$baseMessage(res.msg, 'success')
              this.fetchData()
            }
    ...
    async fetchData () {
      ...
      let res = await tableList(info)
      if (res.status == 200) {
        this.list = res.data
        // 如果当前页没有数据，且当前页大于 1，则跳转上一页重新请求
        if (this.list.length === 0 && this.queryForm.pageNo > 1) {
          this.queryForm.pageNo--
          // 这里先关闭loading，防止loading一直开着
